...
absorb just as well? Answered two ways, not just algebraically asserted:
...
2. INDEPENDENT refit: a fresh intercept is fit (1-D Newton-Raphson on the
   real logistic loss), holding the rule/lstm/gnn coefficients FROZEN at
   their trained values, against the phase-derived ground truth (at_rest =
   normal, everything else = physical disturbance -- the same target
   evaluate_real_hardware.py's own FP/detection-rate numbers use). The
   dumped `label` field is a SECURITY-trust label, constant 1 throughout
   because no attack was performed in these sessions, so it carries no
   information for this check and is deliberately not used as the target.

Both are compared, window by window, against the ORIGINAL flagged decision
(fused < PROCESS_THRESHOLD). This is a verification of the constant-term
argument, not a new training run: the fusion meta-learner itself is never
re-fit, only its scalar intercept, and only against data it was never fit
...
def main():
    ...
    print(f"{len(rows)} windows loaded from {dump_path}")
    ...
    print(f"    flagged mismatches vs original: {exact_mismatches}/{len(rows)}")

    # (2) independent refit against phase-derived ground truth
    ...
    print(f"\n[2] independent refit: intercept = {refit_b:.6f} "
          f"(fit on phase-derived ground truth, rule/lstm/gnn coefficients frozen)")
    print(f"    flagged mismatches vs original: {refit_mismatches}/{len(rows)}")

    verdict = "CONFIRMED" if exact_mismatches == 0 and refit_mismatches == 0 else "NOT CONFIRMED"
    print(f"\n{verdict}: dropping isolation_forest_score and refitting only the "
          ...
          f"every window's flagged/not-flagged outcome on the {split} split.")
    ...
    out = {
        ...
        "independent_refit": {"new_intercept": refit_b, "flagged_mismatches": refit_mismatches},
        "verdict": verdict,
    }
