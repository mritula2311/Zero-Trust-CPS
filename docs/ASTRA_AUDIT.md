...
| P0 | `scripts/train_lstm_ae.py`, `train_transformer.py` | Normal-first filtering and global tick sorting made windows across anomalies, sessions and scenarios. | Validity of learned temporal patterns. | Fixed with `datasets.normal_sequences`; **rebuilt 2026-09-05** — IF → LSTM-AE → Transformer → GNN → fusion → bandit retrained in order on the corrected chain, full 118-test suite green against the new artifacts. Pre-rebuild artifacts archived at `models.archived-20260905_144311/`. |
...
| P1 | `evaluate_gnn_baselines.py::build_snapshots`, set/graph training | Pending node excluded from loss/metrics but neutral 0.9 placeholder still enters graph/attention/pooling. | Missing inputs can influence observed-node predictions. | **Fixed for the GNN specifically, 2026-09-05**: `normalized_adjacency` now masks a pending node out of the adjacency per snapshot (zeroed row/column, degree renormalized), so its placeholder cannot reach a valid neighbour through message passing; verified in `tests/test_gnn_pending_node_masking.py` and `results/gnn_baselines/metrics.json` regenerated (see `docs/CLAIM_EVIDENCE_MATRIX.md` C3). **Still open** for the M6–M9 SetTransformer/DeepSets/GATv2/concat-MLP models in `benchmark_crossdevice_models.py`: their fixed-width/attention/pooling inputs would need an added per-node validity feature and a retrain to mask correctly, which is the "requires redesign and regenerated evidence" scope this row originally meant — not attempted here. |
...
| P0 | `firmware/main.py::connect_mqtt`, config startup | Firmware explicitly disables certificate verification; host configuration can fall back to plaintext/placeholder credentials. | Transport peer identity is not established on firmware. | Host startup refuses plaintext/unconfigured broker auth and template keys cannot authenticate (unchanged). **2026-09-05**: `connect_mqtt()` now supports opting into verification via `device_secrets.MQTT_CA_CERT_FILE` (`cert_reqs=ussl.CERT_REQUIRED` with that CA); unset, it keeps `CERT_NONE` but now prints an explicit unverified-TLS warning at boot instead of connecting silently (`tests/test_firmware_tls_verification.py`). **Not verified against real hardware** — `ussl.wrap_socket`'s `ca_certs` support varies by MicroPython build, the same class of blocker documented for CoAP/DTLS — so this remains a deployment blocker until flash-tested on a board with a real CA. |
...
| P1 | `src/coap_server.py::do_POST` | Unbounded Content-Length/read on a single-thread server exposes availability risk; input framing lacks robust limits. | One client can exhaust endpoint availability. | **Fixed 2026-09-05**: `MAX_BODY_BYTES` (16 KiB) caps the declared and actual body size; a missing/non-numeric `Content-Length` is rejected (400) before any read, an oversized one is rejected (413) before any read (`tests/test_coap_server_hardening.py`). Still open: this caps one request's size, it is not general resource-exhaustion resistance (no rate limit, no connection cap, no read timeout) — no public-load readiness claim. |
...
**2026-09-05 update:** the six-step deployed chain (IF → LSTM-AE → Transformer →
GNN → fusion → bandit) was rebuilt on the corrected `datasets.normal_sequences`
windows; pre-rebuild artifacts archived at `models.archived-20260905_144311/`.
`evaluate_gnn_baselines.py`'s GNN-specific pending-node placeholder leak is
fixed and its benchmark regenerated (`docs/CLAIM_EVIDENCE_MATRIX.md` C3);
`src/coap_server.py` now caps request size; `firmware/main.py` supports opt-in
(untested-on-hardware) CA verification. The M6–M9 set/attention model masking,
research-side provenance/n=15/matched-weighting items, and firmware TLS
verification on actual hardware remain open, below.

- **Software:** firmware certificate verification remains **untested on real
  hardware** (code now supports it, opt-in via `MQTT_CA_CERT_FILE`); M6–M9
  set/attention model masking (SetTransformer/DeepSets/GATv2/concat-MLP in
  `benchmark_crossdevice_models.py` still need an added per-node validity
  feature and a retrain to mask correctly, not attempted); source/run
  provenance; archived artifacts lack training hashes. This is not
  deployment-ready.
