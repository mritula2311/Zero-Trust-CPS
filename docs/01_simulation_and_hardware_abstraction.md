...
> **2026-09-05 audit update, [UPDATED 2026-09-07]:** One fully-captured MPU6050 (`esp32-vib-001`). SW-420 (`esp32-vib-002`) had its first real capture, TRAIN split only, as of 2026-09-05; as of 2026-09-07 it also has held-out VALIDATION/TEST captures (`results/sw420_real_hardware/`, doc 13 Q2) — a **standalone** real-hardware evaluation, separate from its still-pending slot in the constructed 20-node network benchmark below. The legacy live simulator is scoped to its original identities; network research uses generate_network_data.py and explicit provenance (now 20 nodes, grown from 10 — RESULTS §0.13.18).
