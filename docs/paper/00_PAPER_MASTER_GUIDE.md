...
The research benchmark compares M1–M9 on a 20-node constructed heterogeneous network. M6 Set Transformer is the selected standalone relational candidate, with Macro-F1 0.9898 and FPR 0.0025 at its validation-selected operating point. Separately, the gateway now loads a runtime M6 checkpoint and M6-fitted logistic fusion. Offline scenario verification exercises this integration, but the claimed GCN-to-M6 gain lacks a persisted, correctly pinned comparison: the current evaluator loads the promoted M6 fusion for its GCN arm. Do not quote that gain as verified. All numbers resolve to the [master result tables](13_RESULTS_MASTER_TABLES.md).
...
Do not claim twenty physical devices, two MPU6050 devices, physical end-to-end M6 validation, verified runtime-M6 superiority, full reinforcement learning, compliance certification, graph necessity or population-level generator realism. Runtime M6 class weights include inactive placeholders; benchmark masking evidence does not validate this trainer. Read the [claim gate](17_CLAIM_EVIDENCE_MATRIX.md) and [open items](25_OPEN_ITEMS_BEFORE_SUBMISSION.md) before drafting the Abstract.
