...
ZT-Duo authenticates CPS telemetry, evaluates cyber behaviour and physical-process behaviour on separate trust-style scales, and uses both at a policy decision boundary. A compromised identity and an abnormal physical process require different responses. The runtime implements this separation with authenticated-behaviour Security Trust and local detectors plus an M6 (Set Transformer)/logistic Process Trust pipeline (M6 superseded GCN as the deployed relational scorer 2026-09-07; see [17](17_CLAIM_EVIDENCE_MATRIX.md) C03/C05/C15). The four policy actions are ALLOW, ALERT, STEP_UP and BLOCK.

The research benchmark compares M1–M9 on a 20-node constructed heterogeneous network. M6 Set Transformer was the selected **standalone relational candidate** there, with Macro-F1 0.9898 and FPR 0.0025 at its validation-selected operating point -- a separate offline sweep from the live runtime. As of 2026-09-07, a separately-trained M6 checkpoint (on the live runtime's own replay data) is integrated into the runtime fusion, superseding GCN: a held-out synthetic-replay comparison (`scripts/evaluate_ablation_m6.py`) showed higher fused F1/accuracy and fewer false negatives than the prior GCN-based fusion. This is a held-out replay result, not a live-hardware field validation. All numerical claims below resolve to [master result tables](13_RESULTS_MASTER_TABLES.md), where protocols and metric polarity are explicit.
...
Do not claim twenty physical devices, two MPU6050 devices, M6 fusion validated end-to-end against live hardware (only a held-out synthetic-replay comparison exists -- deployed-but-replay-validated is the accurate claim, see [17](17_CLAIM_EVIDENCE_MATRIX.md) C15), full reinforcement learning, formal compliance certification, graph necessity, or population-level generator realism. Read the [claim gate](17_CLAIM_EVIDENCE_MATRIX.md) and [open items](25_OPEN_ITEMS_BEFORE_SUBMISSION.md) before drafting the Abstract.
