...
| Current runtime M6 | `train_set_transformer.py`, three-score TRAIN snapshots | M6 fusion script on VAL_001 | Offline scenario checks; comparative gain not independently pinned | `models/set_transformer_runtime.pt`; inactive-label class-weight and clock mismatch limits |
...
| Historical policy comparison | Frozen GCN-era process chain | `validation_policy_session.json`, VAL_002 seed 4243 | Same 2,933 accepted TEST rows | Six comparators; policy JSON |
