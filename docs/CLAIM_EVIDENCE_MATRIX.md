...
| **Real / Sim / Hybrid** | 20 declared nodes: 10 MPU6050-type and 10 SW-420-type. Two physical identities, but within *this constructed network benchmark* SW-420 has TRAIN observations only; VALIDATION/TEST have one observed physical node, 18 simulated nodes and one pending slot (unaffected by the separate 2026-09-07 standalone SW-420 held-out closure — `results/sw420_real_hardware/` — which does not feed this benchmark's resampling). Raw simulated rows use `source_type=SIMULATED`; LEGACY_SIMULATED is their benchmark provenance category. |
...
| **Real / Sim / Hybrid** | 2 real (`esp32-vib-001` full split; `esp32-vib-002` **TRAIN-only within this network benchmark, VALIDATION/TEST still `PENDING_REAL_HARDWARE_DATA` here**), 18 LEGACY_SIMULATED (equalised 10 MPU6050-type + 10 SW-420-type; the SW-420-type profiles' disturbance magnitudes are anchored to `esp32-vib-002`'s real measured phase means). |
| **Limitation** | `esp32-vib-002` contributes real TRAIN rows only *to this network benchmark*; its VALIDATION/TEST network records are still `PENDING_REAL_HARDWARE_DATA` and excluded from every network-benchmark metric. So the network as *evaluated* on VALIDATION/TEST is 1 fully-real + 18 simulated + 1 TRAIN-only-real/pending-elsewhere. RESULTS.md §0.13.18.2/§0.13.19 measured a concrete cost of this gap and corrected an earlier misdiagnosis of it. **[2026-09-07]** A separate, standalone `esp32-vib-002` held-out evaluation now exists outside this network-resampling pipeline (`results/sw420_real_hardware/`: 0/70 and 0/108 resting FP, 109/109 and 115/115 detection) — it does not close this network benchmark's own pending slot. |
