...
| FR-P2 | The ensemble MUST include a relational signal (graph or set/concatenated alternative) capable of detecting cross-device anomalies | Current constructed Task-2 TEST accuracy: global count B0 0.3958, concat logistic B1 **0.5433**, concat MLP B2 0.5267, GNN 0.5375. B0 already sees all valid nodes through a count; a literal single-device comparison is pending. Task-1 B2 F1 fell materially from **0.9662 to 0.9174** after masking correction; GNN F1 is 0.5865, above B3 0.3082 but below B0/B1/B2. No statistical superiority or causal network-size claim. Historical runs and current evidence: `docs/PAPER_GNN_BASELINE_VERIFICATION.md`, RESULTS §0.13.25, C2/C3. | **Signal present; indexed-versus-count observation supported; graph superiority unestablished** |
...
- **The evaluated GCN does not establish graph superiority.** Corrected
  Task-1 TEST F1: B0 0.9708, B1 0.7371, B2 0.9174, GNN 0.5865, B3 0.3082.
  B2 fell from 0.9662 after the pending-content correction. Task-2 TEST
  accuracy: B1 0.5433, GNN 0.5375, B2 0.5267, B0 global count 0.3958.
  GNN beats B3 on Task 1 and B0/B2 on Task 2. C2 is a scoped indexed-versus-
  global-count observation; B0 Task 2 is not a single-node control. Network
  cardinality, sensor mix, provenance and model-chain changes remain
  confounded. The distinct historical runs are attributed in
