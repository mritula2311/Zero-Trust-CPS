...
class SetTransformerScorer:
    ...
    def _load(self):
        import os
        if not os.path.exists(SET_TRANSFORMER_MODEL_PATH):
            return
        model = _SetTransformer()
        model.load_state_dict(torch.load(SET_TRANSFORMER_MODEL_PATH, map_location=_TORCH_DEVICE, weights_only=True))
        model.eval()
